...
function filesUploaded($files, $field)
{
    ...
    for ($i = 0; $i < count($files['name']); $i++) {
        ...
        $target_path = $configs['upload_dir'].basename($files['name'][$i]);
...
function removeUploadsFromServer()
{
    ...
    foreach ($_FILES as $files) {
        if (!empty($files['name'][0])) {
            for ($i = 0; $i < count($files['name']); $i++) {
                $file = $configs['upload_dir'].basename($files['name'][$i]);
...
function sendEmailWithInformations()
{
    ...
    foreach ($_FILES as $files) {
        if (!empty($files['name'][0])) {
            for ($i = 0; $i < count($files['name']); $i++) {
                // Define file path
                $path = __DIR__.'/'.$configs['upload_dir'].$files['name'][$i];
...
function getPlainEmailContent()
{
    ...
    foreach ($fields as $field) {
        ...
        $content .= $field['display_name'].': '.$value.PHP_EOL;
    }
...
function sendConfirmationEmailToUser()
{
    ...
    // Set content
    $mail->Body = $lang->emails_info->confirmation->body;
